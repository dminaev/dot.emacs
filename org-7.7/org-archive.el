...
(defun org-archive-to-datetree ()
  "Archive the current heading to datetree structure "
  (interactive)
  (save-excursion
    (setq file (abbreviate-file-name
	  ...
	  dct (decode-time ct)
	  date-to-file (list (nth 4 dct) (nth 3 dct) (nth 5 dct))
	  buffer (or (find-buffer-visiting afile) (find-file-noselect afile)))
    (unless afile
      (error "Invalid `org-archive-location'"))
    (org-copy-subtree)
    (set-buffer buffer)
    (org-datetree-find-date-create date-to-file)
    (end-of-line 1)
    (insert "\n**** ")
