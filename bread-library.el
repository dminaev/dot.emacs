...
(defun bread-library-by-title ()
  ...
  (unless (eq bread-library-sort 'title)
    (message "lib by title")
    (bread-library-save-db (bread-library-org-to-sexp (current-buffer)))
    (setq bread-library-sort 'title)
    (message "going to erase buffer")
    (org-mode)
    (erase-buffer)
    (message "erased")
    (bread-library-display-library)
...
(defun bread-library-org-to-sexp (buffer)
  ...
  (save-excursion
    (save-restriction
      ...
      (let ((result nil))
	(goto-char (point-min))
	(while (search-forward ":ENTRYMARKEDDIRTY:" nil t)
	  (let ((props (org-entry-properties)))
	    (when (assoc "D" props)
	      (let ((url (cdr (assoc "url" props)))
		    (author (cdr (assoc "author" props)))
		...
		(when (assoc url bread-library-db)
		  (setq bread-library-db (delete (assoc url bread-library-db) bread-library-db)))
		(setq bread-library-db (cons (list url author title tags state genre priority desc) bread-library-db))))))
	bread-library-db))))
...
(defun bread-library-mark-entry-dirty (beg end length)
  ...
  (save-excursion
    (unless (org-entry-get nil "D" "t")
      (org-entry-put nil "ENTRYMARKEDDIRTY"  "t"))))